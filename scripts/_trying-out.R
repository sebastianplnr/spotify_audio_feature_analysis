...
data2$year_week = with(data2, paste0(year, "-", week))
...
valence_heatmap = data2 %>%
  ...
  mutate(difference = round((valence_pred - avg_weighted_valence), digits = 2),
         perc = round((difference/valence_pred)*100, digits = 2)) %>%
  ggplot(aes(year_week, country, fill = perc)) +
  geom_tile() +
...
data2 %>%
  ...
  mutate(difference = round((valence_pred - avg_weighted_valence), digits = 2),
         perc = round((difference/valence_pred)*100, digits = 2)) %>%
  ggplot(aes(as.numeric(week), perc, colour = country)) +
  geom_line() + ylim(-17, 17)
